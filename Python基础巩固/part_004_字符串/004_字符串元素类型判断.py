...
e = ' 234234   FDSFDS   '  # 只要字母都是大写就行，非字母不管
result = e.isupper()
print(result)

print("*" * 20, "str.islower()：判断字符串是否【字母全为小写字母】，可以有字母以外的字符", "*" * 20)
f = ' 234234   fdsfsdsfds   '  # 只要字母都是小写就行，非字母不管
result = f.islower()
print(result)
...
# 手机号码：纯数字，长度11（目前写死：15811119999）
# 要求输入玩用户名输入框立即验证，验证通过才允许输入密码
# 密码：必须6位数字（目前写死：200325）
    ...
    if name.isdigit() and len(name) == 11 or name.islower() and not name[0].isdigit() and len(name) > 6:
        ...
        while True:
            ...
            if passwd.isdigit() and len(passwd) == 6:
                ...
                if (name == "admin123" or name == "15811119999") and passwd == "200325":
                    print("登录成功！")
                    hello = False  # 控制外层循环
